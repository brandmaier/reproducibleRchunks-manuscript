...
abstract: |
 Research is considered _reproducible_ if the same analysis on the same data yields the same results if performed on a different computer or on the same computer later in time. Reproducibility is a prerequisite for replicable, robust and transparent research.
...
Furthermore, the content of the code chunks itself (that is, the code syntax) is also fingerprinted and stored, such that later non-reproduction can be traced back to either changes of the code chunk  or changes of the computation.
...
A reproducibility report is shown below the code chunk to indicate all variables of the given chunk whose data are stored:
...
![](img/generation-step2-failed.png)
...
numbers_ex <- rnorm(5)
```
Here is an example of how the contents are stored, which is a single variable called `numbers` with a vector of five numbers `r numbers_ex` generated by the following command:

```{r echo=TRUE, eval=FALSE}
numbers_ex <- rnorm(5)
```

This vector is serialized to a JSON file, which is an open and transparent format for storing contents that are both machine-readable but also human-readable:
