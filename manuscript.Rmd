...
The `reproducibleRchunk` package allows users to make computational results in R Markdown documents automatically testable for successful reproduction (does the same script with the same data produce the same results, e.g. on a different computer and/or later in time) with minimal changes to the workflow.
There are only two things users need to change in their Markdown document if they are already using R Markdown: In the first code chunk of their document, they load the package and then change the code chunk type from `r` to `reproducibleR` for all code chunks for which reproduction should be automatically verified in future.
