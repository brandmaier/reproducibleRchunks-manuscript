...
The JSON format has a clear, formal structure, which allows easy parsing of the information both for computers and humans, which can ease "forensics" in case reproduction fails.
Such forensic investigation is further facilitated, the package also stores the code syntax.
...
<!---AP: I think this section could be removed: start--->
...
content <- digest::digest(genesis,algo = "sha256")
content <- gsub(pattern="\\n",replacement="  \n", x=content)
cat(content)
```
\end{verbatim}
...
<!---AP: I think this section could be removed: end--->
...
<!---I think this is sound advice and should be kept--->
...
Using exact comparisons for testing complex object identity is sometimes more restrictive than one would expect.
Complex objects (like the fit objects of statistical models, or plots), often contain information that changes on each run, like time stamps, surprising randomness, numerical instability etc. which makes the object technically not identical but would rarely count as irreproducible.
Another complication is numeric comparisons. 
