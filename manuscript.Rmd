...
When ever the document is compiled (or, _knitted_ in R speak), all of its computer code chunks are executed and thus generate dynamic content in the document.
This is particularly interesting for psychological research reports, as the `papaja` package [@R-papaja] allows R Markdown documents (including tables and figures) to be automatically formatted in APA style; most importantly, statistical results and content of tables and figures are dynamically generated.
...
```{r jsonschema, fig.cap="A JSON file stores fingerprints of all newly declared variables in a reproducibleR-chunk as well as fingerprints of the code chunk syntax. This image by Andreas Brandmaier and Anke Schneider is licensed under CC BY 4.0"}
...
Using exact comparisons for testing object identity is sometimes more restrictive than one would expect.
...
<!--
```{reproducibleR helloworld, echo=TRUE, eval=TRUE}
set.seed(42)
x <- rnorm(10, mean=0, sd=1) 
y <- 4 * 4
```
-->
```{r fig.cap="A reproducibleR chunk with two variables automatically tested for reproducibility."}
knitr::include_graphics("img/helloworld.png")
```
...
- reproducibleRchunks.filetype: This is the type of data storage. Currently, we only support the 'json' format; however, this leaves room for other formats to be supported in future. 
- reproducibleRchunks.hashing: Boolean. Indicates whether fingerprints should be used (default) or raw values should be stored.
- reproducibleRchunks.hashing_algorithm: This is the hashing algorithm, which is used to generate fingerprints of variable contents. By default, the algorithm offered by the `digest` package are used.
- reproducibleRchunks.templates: A list with keys corresponding to pandoc output formats and values to templates for formatting the reproducibility reports. See below for more details.
