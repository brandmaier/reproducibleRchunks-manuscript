...
abstract: |
 ...
 However, a simple way to detect non-reproducibility, that is, unwanted changes in these assets over time is still missing.
...
For illustration, here is the content of the `cpuTime` variable and the `timestump` variable for the aforementioned factor model, which was estimated when this manuscript was generated:
...
Friends of the tidyverse approach [@wickham2019welcome] are probably well advised to use functions `tidy()` and `glance()` from the `broom` [@broom] package, which will write parameter estimates and model fit statistics from various statistical models in R (e.g., anova, glm, coxph, gam, lm, lavaan,smooth.spline, survfit, and others) into a consistent and easy accessible format (a so-called _tibble_), which lends itself for fingerprinting with our suggested approach.
