...
Adepts of the tidyverse approach [@wickham2019welcome] are probably well advised to use functions `fit()` and `glance()` from the `broom` [@broom] package, which will write parameter estimates and model fit statistics from various statistical models in R (e.g., anova, glm, coxph, gam, lm, lavaan,smooth.spline, survfit, and others) into a consistent and easy accessible format, which lends itself for fingerprinting with our suggested approach.
