...
Packages like `papaja` package [@R-papaja] further extend R Markdown's capabilities to format documents (including tables and figures) in a specific journal style, such as APA style.
...
# sha benchmark
set.seed(345)
benchmark <- microbenchmark::microbenchmark(digest::sha1(numbers), unit = "seconds", check = "equal", setup = {numbers <- runif(1000)})
```
...
To reduce chance of collisions, the package defaults to `sha256` which results in fingerprints of 64 hexadecimal characters, which corresponds to a 256-bit fingerprint but comes in cost of some speed (`sha256` of a vector of 100 numbers takes `r mean(benchmark$time)` nanoseconds).
